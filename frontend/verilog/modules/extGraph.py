    ...
    def exportDOT(self, varName: str = None, params: dict = {}):
        ...
        if extractCIP:
            ...
            for node in self.graph.nodes():
                ...
                if re.match(r".*address.*", node.attr['label']):
                    try:
                        outNode = self.graph.get_node(node.attr['label'].replace('address', 'out'))
                    except KeyError:
                        outNode = None
                    try:
                        inNode = self.graph.get_node(node.attr['label'].replace('address', 'in'))
                    except KeyError:
                        inNode = None
                    CIPpairs.append((node, outNode, inNode))

            for i, (addressNode, outNode, inNode) in enumerate(CIPpairs):
                cipGraph = self.graph.add_subgraph(name=f'cluster_cip{i}', label='CIP', style='dashed', color='green')
        ...
        if extractREG:
            ...
            for stage, nodes in stages.items():
                regGraph = self.graph.add_subgraph(name=f'cluster_reg{stage}', label=f'pipeline stage \#{stage}', style='dashed', color='orange')
