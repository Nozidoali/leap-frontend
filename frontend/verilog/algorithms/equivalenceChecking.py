...
def netlistsAreEqual(netlist1: Netlist, netlist2: Netlist, verbose: bool = True) -> bool:
    if len(netlist1.getModules()) != len(netlist2.getModules()):
        print(f"Number of modules are not equal: {len(netlist1.getModules())} != {len(netlist2.getModules())}")
        return False
    # compare modules
    for module in netlist1.getModules():
        if module not in netlist2.getModules():
            ...
            return False
        if not modulesAreEqual(netlist1.getModule(module), netlist2.getModule(module), verbose):
            print(f"Module {module} in netlist1 is not equal to module {module} in netlist2")
            return False
    return True
...
def modulesAreEqual(module1: Module, module2: Module, verbose: bool = True) -> bool:
    ...
    for port in module1.getPortNames():
        ...
        if module1.getPort(port) != module2.getPort(port):
            print(f"Port {port} in module {module1.getName()} is not equal to port {port} in module {module2.getName()}")
            return False
    return True